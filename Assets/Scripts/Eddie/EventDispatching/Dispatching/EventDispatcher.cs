...
namespace Eddie.EventDispatching.Dispatching
{
    public class EventDispatcher : IEventDispatcher
    {
        ...
        private void Process(IEvent ev, IEnumerable<Group> groups)
        {
            foreach (var group in groups)
            {
                ...
                if (group.IsSequential)
                    ProcessSequence(ev, group).Forget();
                else
        ...
        private async UniTaskVoid ProcessSequence(IEvent ev, Group group)
        {
        ...
        private void ProcessEvent(IEvent ev, Type handlerType, Group group)
        {
            ...
            switch (handlerBase)
            {
                ...
                {
                    RunAndForget(taskHandler, group).Forget();
                    break;
                }
            }
        }

        //If a task is not awaited, then any internal exception will be hidden
        private async UniTaskVoid RunAndForget(ITaskHandler taskHandler, Group group)
        {
            var (token, onTaskFinished) = GetToken(group.CancelOnSceneSwitch);
            try
            {
